...
import '../../features/seller_pages/listining_seller/presentation/pages/property_details.dart';
import '../../features/seller_pages/listining_seller/presentation/pages/show_dialog_rate_screen.dart';
import '../../features/seller_pages/notifications_seller/data/models/push_notification_mode.dart';
import '../../features/seller_pages/payment/presentation/pages/screen_select_shotting_dailog.dart';
...
void routePushNotification({PushNotification? pushNotification}) {
  if (pushNotification!.destinationName == "Property") {
    SplashScreen.navigatorKey.currentState!.push(MaterialPageRoute(
        builder: (context) =>
            PropertyDetails(listingId: pushNotification.destinationId!)));
  } else if (pushNotification.destinationName == "Home") {
    SplashScreen.navigatorKey.currentState!
        .push(MaterialPageRoute(builder: (context) => HomeMainSeller()));
  } else if (pushNotification.destinationName == "RateShooting") {
    //   locator<NotificationService>().navigateTo(
    //       BrandDetilsPage(id: int.parse(pushNotification.destinationId!)));
    SplashScreen.navigatorKey.currentState!.push(MaterialPageRoute(
        builder: (context) => ShowDialogRateScreen(
              propertyId: int.parse(pushNotification.destinationId!),
            )));
  } else if (pushNotification.destinationName == "ShootingDate") {
    //   locator<NotificationService>().navigateTo(
    //       BrandDetilsPage(id: int.parse(pushNotification.destinationId!)));
    SplashScreen.navigatorKey.currentState!.push(MaterialPageRoute(
        builder: (context) => ScreenSelectShottingDateDialog(
              listingId: pushNotification.destinationId!,
            )));
  } else if (pushNotification.destinationName == "Chat") {
    SplashScreen.navigatorKey.currentState!.push(MaterialPageRoute(
        builder: (context) => ChatScreen(
              channelID: int.parse(pushNotification.destinationId!),
              name: pushNotification.name!,
              imagePath: pushNotification.profilePath ?? "",
            )));
  }
}
