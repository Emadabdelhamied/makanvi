...
class SelectedSingleChipListWidget extends StatefulWidget {
  const SelectedSingleChipListWidget({
    Key? key,
  ...
  final List<TypeProparty> listOfChipNames;
  ...
  /// ensure that you wrap the [SelectedSingleChipListWidget]
  ...
  /// the [SelectedSingleChipListWidget], which is used in case of
  ...
  @override
  _SelectedSingleChipListWidgetState createState() =>
      _SelectedSingleChipListWidgetState();
}

class _SelectedSingleChipListWidgetState
    extends State<SelectedSingleChipListWidget> {
  bool _checkChipSelectionStatus(int _index) {
    if (widget.listOfChipIndicesCurrentlySeclected.first == _index) {
      return true;
    }
    ...
    return Wrap(
      children: List.generate(
        ...
        (index) => Padding(
          ...
          child: ChoiceChip(
            label: Text(
              widget.listOfChipNames[index].title,
              style: widget.style != null
                  ? widget.style!.copyWith(
                      color: _textColorizer(widget.listOfChipNames[index].id),
                    )
            ...
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(
                widget.borderRadiiList.length == 1
                    ...
                    : widget.borderRadiiList[widget.listOfChipNames[index].id],
              ),
              side: BorderSide(
                color: _borderColorizer(widget.listOfChipNames[index].id),
              ),
            ),
            backgroundColor: widget.inactiveBgColorList.length == 1
                ? widget.inactiveBgColorList.first
                : widget.inactiveBgColorList[widget.listOfChipNames[index].id],
            selected:
                _checkChipSelectionStatus(widget.listOfChipNames[index].id),
            selectedColor: _tileColorizer(widget.listOfChipNames[index].id),
            onSelected: (val) {
              // update other chips depending on value of [supportsMultiSelect]
              _handleOnSelected(widget.listOfChipNames[index].id);
